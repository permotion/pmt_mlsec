"""
Preprocessing pipeline — Modelo A (CSIC 2010) — v2

Lee: data/raw/csic2010/csic_database.csv
Escribe: data/processed/csic2010/features_v2.parquet

Cambios respecto a v1 (ver docs/experiments.md):
- Agrega url_pct_density: ratio de chars encoded (%XX) sobre longitud total de URL
- Agrega url_param_count: cantidad de parámetros en la URL (conteo de '=')

Por qué:
- El análisis de FP en csic2010_feature_analysis_v1.ipynb mostró que los 1.886 FP
  no tienen indicadores de payload — el modelo los clasifica como ataque solo por
  tener URLs largas. url_pct_density y url_param_count dan contexto a esa longitud.
- Correlación con label: url_pct_density=0.267, url_param_count=0.146
- Impacto medido en v1: Precision 0.655 → 0.704 (+0.049), ROC-AUC 0.939 → 0.950

Descartadas en este ciclo:
- url_has_traversal: NaN — '../' nunca aparece literal, siempre percent-encoded
- post_has_pct27: NaN — intersección insuficiente

Decisiones heredadas de v1 (docs/eda.md):
...
OUT_PATH = OUT_DIR / "features_v2.parquet"
...
def preprocess(raw_path: Path = RAW_PATH, out_path: Path = OUT_PATH) -> pd.DataFrame:
    ...
    # Columnas finales — v2 agrega url_param_count y url_pct_density
    feature_cols = [
